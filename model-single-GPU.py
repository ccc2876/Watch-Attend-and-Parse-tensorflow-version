...
class Watcher_train():
    ...
    def dense_net(self, input_x, mask_x):
        ...
        x = tf.layers.conv2d(x, filters=self.input_conv_filters, strides=self.input_conv_stride,
                             kernel_size=self.input_conv_kernel, padding='SAME', data_format='channels_last',
                             use_bias=False,
                             kernel_initializer=tf.random_uniform_initializer(-limit, limit, dtype=tf.float32))
        mask_x = mask_x[:, 0::2, 0::2]
        x = tf.layers.batch_normalization(x, training=self.training, momentum=0.9, scale=True,
                                          gamma_initializer=tf.random_uniform_initializer(
                                              -1.0 / math.sqrt(self.input_conv_filters),
                                              1.0 / math.sqrt(self.input_conv_filters), dtype=tf.float32),
                                          epsilon=0.0001)
        x = tf.nn.relu(x)
        ...
        for i in range(self.blocks):
            for j in range(self.level):
                ...
                x = tf.layers.conv2d(x, filters=4 * self.growth_rate, kernel_size=[1, 1],
                                     strides=1, padding='VALID', data_format='channels_last', use_bias=False,
                                     kernel_initializer=tf.random_uniform_initializer(-limit, limit, dtype=tf.float32))
                x = tf.layers.batch_normalization(inputs=x, training=self.training, momentum=0.9, scale=True,
                                                  gamma_initializer=tf.random_uniform_initializer(
                                                      -1.0 / math.sqrt(4 * self.growth_rate),
                                                      1.0 / math.sqrt(4 * self.growth_rate), dtype=tf.float32),
                                                  epsilon=0.0001)
                x = tf.nn.relu(x)
                ...
                x = tf.layers.conv2d(x, filters=self.growth_rate, kernel_size=[3, 3],
                                     strides=1, padding='SAME', data_format='channels_last', use_bias=False,
                                     kernel_initializer=tf.random_uniform_initializer(-limit, limit, dtype=tf.float32))
                x = tf.layers.batch_normalization(inputs=x, training=self.training, momentum=0.9, scale=True,
                                                  gamma_initializer=tf.random_uniform_initializer(
                                                      -1.0 / math.sqrt(self.growth_rate),
                                                      1.0 / math.sqrt(self.growth_rate), dtype=tf.float32),
                                                  epsilon=0.0001)
                x = tf.nn.relu(x)
                x = tf.layers.dropout(inputs=x, rate=self.dropout_rate, training=self.training)
            ...
            if i < self.blocks - 1:
                ...
                x = tf.layers.batch_normalization(x, training=self.training, momentum=0.9, scale=True,
                                                  gamma_initializer=tf.random_uniform_initializer(
                                                      ...
                                                      1.0 / math.sqrt(self.dense_channels), dtype=tf.float32),
                                                  epsilon=0.0001)
                x = tf.nn.relu(x)
                x = tf.layers.dropout(inputs=x, rate=self.dropout_rate, training=self.training)
...
class Attender():
    def __init__(self, channels,  # output of Watcher | [batch, h, w, channels]
        ...
        self.U_f = tf.Variable(norm_weight(self.coverage_filters, self.dim_attend),
                               name='U_f')  # $U_f x f_i$ | [cov_filters, dim_attend]
        self.U_f_b = tf.Variable(np.zeros((self.dim_attend,)).astype('float32'),
                                 name='U_f_b')  # $U_f x f_i + U_f_b$ | [dim_attend, ]

        self.U_a = tf.Variable(norm_weight(self.channels,
                                           self.dim_attend),
                               name='U_a')  # $U_a x a_i$ | [annotatin_channels, dim_attend]
        self.U_a_b = tf.Variable(np.zeros((self.dim_attend,)).astype('float32'),
                                 name='U_a_b')  # $U_a x a_i + U_a_b$ | [dim_attend, ]

        self.W_a = tf.Variable(norm_weight(self.dim_decoder,
                                           self.dim_attend), name='W_a')  # $W_a x h_{t_1}$ | [dec_dim, dim_attend]
        self.W_a_b = tf.Variable(np.zeros((self.dim_attend,)).astype('float32'),
                                 name='W_a_b')  # $W_a x h_{t-1} + W_a_b$ | [dim_attend, ]

        self.V_a = tf.Variable(norm_weight(self.dim_attend, 1), name='V_a')  # $V_a x tanh(A + B + C)$ | [dim_attend, 1]
        self.V_a_b = tf.Variable(np.zeros((1,)).astype('float32'),
                                 name='V_a_b')  # $V_a x tanh(A + B + C) + V_a_b$ | [1, ]

        self.alpha_past_filter = tf.Variable(conv_norm_weight(1, self.dim_attend, self.coverage_kernel),
...
class Parser():
    ...
    def __init__(self, hidden_dim, word_dim, attender, context_dim):
        ...
        self.W_yz_yr = tf.Variable(np.concatenate(
            ...
            name='W_yz_yr')  # [dim_word, 2 * dim_decoder]
        ...
        self.U_hz_hr = tf.Variable(np.concatenate(
            ...
            name='U_hz_hr')  # [dim_hidden, 2 * dim_hidden]
        ...
        self.U_hz_hr_nl = tf.Variable(np.concatenate(
            [ortho_weight(self.hidden_dim), ortho_weight(self.hidden_dim)], axis=1),
            name='U_hz_hr_nl')  # [dim_hidden, 2 * dim_hidden] non_linear

        self.b_hz_hr_nl = tf.Variable(np.zeros((2 * self.hidden_dim,)).astype('float32'),
                                      name='b_hz_hr_nl')  # [2 * dim_hidden, ]
    ...
    def get_ht_ctx(self, emb_y, target_hidden_state_0, annotations, a_m, y_m):
        ...
        res = tf.scan(self.one_time_step, elems=(emb_y, y_m),
                      initializer=(target_hidden_state_0,
                                   ...
                                   tf.zeros(
                                       [tf.shape(annotations)[0], tf.shape(annotations)[1], tf.shape(annotations)[2]]),
                                   tf.zeros(
                                       [tf.shape(annotations)[0], tf.shape(annotations)[1], tf.shape(annotations)[2]]),
                                   annotations, a_m))
    ...
    def one_time_step(self, tuple_h0_ctx_alpha_alpha_past_annotation, tuple_emb_mask):
        ...
        context, alpha, alpha_past_one = self.attender.get_context(annotation_one, pre_h, alpha_past_one,
                                                                   a_mask)  # [batch, dim_ctx]
...
class WAP():
    ...
    def get_cost(self, cost_annotation, cost_y, a_m, y_m):
        ...
        emb_shift = tf.concat([emb_pad, tf.strided_slice(emb_y, [0, 0, 0], [-1, batch_size, self.word_dim], [1, 1, 1])],
                              axis=0)
        new_emb_y = emb_shift
        anno_mean = tf.reduce_sum(cost_annotation * a_m[:, :, :, None], axis=[1, 2]) / tf.reduce_sum(a_m, axis=[1, 2])[
                                                                                       :, None]
    ...
    def get_word(self, sample_y, sample_h_pre, alpha_past_pre, sample_annotation):
        ...
        context, _, alpha_past = self.parser.attender.get_context(sample_annotation, pre_h, alpha_past_pre,
                                                                  None)  # [batch, dim_ctx]
...
def main(args):
    ...
    train, train_uid_list = dataIterator(args.path + '/data/offline-train.pkl', args.path + '/data/train_caption.txt',
                                         worddicts,
                                         batch_size=args.batch_size, batch_Imagesize=400000,
                                         maxlen=100, maxImagesize=400000)

    valid, valid_uid_list = dataIterator(args.path + '/data/offline-test.pkl', args.path + '/data/test_caption.txt',
                                         worddicts,
                                         batch_size=args.batch_size, batch_Imagesize=400000,
                                         maxlen=100, maxImagesize=400000)
    ...
    anno = tf.placeholder(tf.float32, shape=[None, annotation.shape.as_list()[1], annotation.shape.as_list()[2],
    ...
    w = WAP(watcher_train, attender, parser, 256, 256, annotation.shape.as_list()[3], 111, if_trainning)

    hidden_state_0 = tf.tanh(
        tf.tensordot(tf.reduce_mean(anno, axis=[1, 2]), w.Wa2h, axes=1) + w.ba2h)  # [batch, hidden_dim]
        ...
        for epoch in range(max_epoch):
            ...
            for batch_x, batch_y in train:
                ...
                cost_i, _ = sess.run([cost, trainer],
                                     feed_dict={x: batch_x, y: batch_y, x_mask: batch_x_m, y_mask: batch_y_m,
                                                if_trainning: True, lr: lrate})
                ...
                if np.mod(uidx, dispFreq) == 0:
                    ...
                    log.write('Epoch ' + str(epoch) + ' Update ' + str(uidx) + ' Cost ' + str(cost_s) + ' Lr ' + str(
                        lrate) + '\n')
                    log.flush()
                    cost_s = 0
                ...
                if np.mod(uidx, sampleFreq) == 0:
                    ...
                    for batch_x, batch_y in valid:
                        for xx in batch_x:
                            ...
                            sample, score = w.get_sample(p, w, h, alpha, annot, h_state,
                ...
                if np.mod(uidx, validFreq) == 0:
                    ...
                    for batch_x, batch_y in valid:
                        batch_x, batch_x_m, batch_y, batch_y_m = prepare_data(batch_x, batch_y)
                        pprobs, annot = sess.run([cost, annotation],
                                                 feed_dict={x: batch_x, y: batch_y, x_mask: batch_x_m,
                                                            y_mask: batch_y_m, if_trainning: False})
                        probs.append(pprobs)
                    ...
                    log.write(
                        'Valid WER: %.2f%%, ExpRate: %.2f%%, Cost: %f' % (valid_per, valid_sacc, valid_err_cost) + '\n')
                    log.flush()
            if estop:
                break
